...
enum storeTypes {PROG_NAME, INTEGER, BOOLEAN};//accepted storage types
...
struct entry                                 //struct for symbol table entrys
{
   //entry vars
    ...
    //entry constructor
    entry(string iN, string eN, storeTypes dT, modes m, string v, allocation a, int u) :
...
void VarStmts();              //Ids(),':',storeTypes,";"
string Ids();                 //NON_KEY_ID,NON_KEY_ID,...
void BeginEndStmt();          //'begin','end','.'

void Insert(string,storeTypes,modes,string,allocation,int);  //insert new entry to symbolTable
string GenInternalName(storeTypes);                          //Generates internal storage name
storeTypes WhichType(string);                                //determines data type of string
...
string GenInternalName(storeTypes t)
{
   string name;
   switch(t)
   {
